<?php
	...
	HeaderingExcel('LIST_DAFTAR_PAPER_DARI_CALON_PEMAKALAH_(POSTER_PRESENTATION)_YANG_TELAH_DITERIMA_YANG_TELAH_DI_DOWNLOAD.xls');
	...
	*/
	...
	$worksheet1->write_string(2,1,"LIST DAFTAR PAPER DARI CALON PEMAKALAH (POSTER PRESENTATION) YANG TELAH DITERIMA YANG TELAH DI DOWNLOAD",$fBesar);
	...
	$worksheet1->write_string(4,7,"Status",$fList_title);
	$worksheet1->write_string(4,8,"First Name",$fList_title);
	$worksheet1->write_string(4,9,"Last Name",$fList_title);
	$worksheet1->write_string(4,10,"Institution",$fList_title);
	$worksheet1->write_string(4,11,"Address",$fList_title);
	$worksheet1->write_string(4,12,"City",$fList_title);
	$worksheet1->write_string(4,13,"Country",$fList_title);
	$worksheet1->write_string(4,14,"E-mail",$fList_title);
	$worksheet1->write_string(4,15,"Telephone",$fList_title);
	$worksheet1->write_string(4,16,"Fax",$fList_title);
	$worksheet1->write_string(4,17,"Mobile Phone",$fList_title);
		
	//tabel content======================================================================================
	$i=1;
	$d = mysql_query("select a.id_abstrack, p.username, p.nama, p.institusi, p.negara, a.title, a.jenis from tbl_peserta p, tbl_abstrack a, tbl_paper r where p.id_peserta=a.id_peserta and a.terima=1 and a.jenis=2 and r.download=1 and a.id_abstrack=r.id_abstrack order by p.username");
	while($d1 = mysql_fetch_array($d))
	{
		...
		$worksheet1->merge_cells(4+$j,7,4+$j+$c2,7);
		if($d1["jenis"] == 1)
			$worksheet1->write_string(4+$j,7,"Full Paper",$fList);
		else if($d1["jenis"] == 2)
			$worksheet1->write_string(4+$j,7,"Poster",$fList);
		for($f=1; $f<=$c2; $f++)
			$worksheet1->write_blank(4+$j+$f,7,$fList);
		
		$k=0;
		while($c1 = mysql_fetch_array($c))
		{
			$worksheet1->write_string(4+$j+$k,8,$c1[2],$fList);
			$worksheet1->write_string(4+$j+$k,9,$c1[3],$fList);
			$worksheet1->write_string(4+$j+$k,10,$c1[4],$fList);
			$worksheet1->write_string(4+$j+$k,11,$c1[5],$fList);
			$worksheet1->write_string(4+$j+$k,12,$c1[6],$fList);
			$worksheet1->write_string(4+$j+$k,13,$c1[7],$fList);
			$worksheet1->write_string(4+$j+$k,14,$c1[8],$fList);
			$worksheet1->write_string(4+$j+$k,15,$c1[9],$fList);
			$worksheet1->write_string(4+$j+$k,16,$c1[10],$fList);
			$worksheet1->write_string(4+$j+$k,17,$c1[11],$fList);
